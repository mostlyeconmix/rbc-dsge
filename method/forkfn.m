function fork = forkfn(bigthet);
% Uses the Kalman filter to generate k-step-ahead forecasts for output,
%   consumption, investment, and hours worked from the real business
%   cycle model with indivisible labor
%
...
% generate forecasts of transformed variables
  ...
  ytk = biggx*(bigfx^(k-1))*xt;

% generate forecasts of untransformed variables

  bigytk = ytk(1) + (bigt+k)*log(eta) + log(yss);

  bigctk = ytk(2) + (bigt+k)*log(eta) + log(css);

  bigitk = log(exp(bigytk)-exp(bigctk));

  bightk = ytk(3) + log(hss);

  fork = [ bigytk bigctk bigitk bightk ];